...
export default function AssetList() {
  ...
  const getRecommendationBadge = (rec) => {
    switch(rec) {
      case 'Dispose':
        return <span className="px-2 py-0.5 rounded-md text-xs font-semibold bg-red-950/40 border border-red-800/40 text-red-400 flex items-center gap-1 w-max">⚠️ Dispose</span>;
      case 'Reallocate':
        return <span className="px-2 py-0.5 rounded-md text-xs font-semibold bg-amber-950/40 border border-amber-800/40 text-amber-400 flex items-center gap-1 w-max">⚙️ Reallocate</span>;
      case 'Repair':
        return <span className="px-2 py-0.5 rounded-md text-xs font-semibold bg-orange-950/40 border border-orange-800/40 text-orange-400 flex items-center gap-1 w-max">🔧 Repair</span>;
      case 'Allocate':
        return <span className="px-2 py-0.5 rounded-md text-xs font-semibold bg-emerald-950/40 border border-emerald-800/40 text-emerald-400 flex items-center gap-1 w-max">➕ Allocate</span>;
      default:
        return <span className="px-2 py-0.5 rounded-md text-xs font-semibold bg-slate-900 border border-slate-800 text-slate-400 flex items-center gap-1 w-max">✓ Optimal</span>;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white font-medium px-5 py-2.5 rounded-xl shadow-lg shadow-violet-500/15 hover:shadow-violet-500/25 transition-all duration-300 hover:-translate-y-0.5 text-sm cursor-pointer"
        >
          <Plus size={16} />
          Register Asset
        </button>
      ...
      <div className="glass-card rounded-2xl p-4 space-y-4">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          {/* View mode toggle */}
          <div className="flex items-center gap-2 bg-slate-900/60 border border-slate-800/80 p-1 rounded-xl w-full md:w-auto justify-center md:justify-start">
            <button
              onClick={() => setViewMode('grid')}
              className={`p-2 rounded-lg transition-all duration-200 cursor-pointer ${viewMode === 'grid' ? 'bg-slate-800 text-violet-400 shadow-sm' : 'text-slate-400 hover:text-white'}`}
            >
              <Grid size={16} />
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={`p-2 rounded-lg transition-all duration-200 cursor-pointer ${viewMode === 'list' ? 'bg-slate-800 text-violet-400 shadow-sm' : 'text-slate-400 hover:text-white'}`}
            >
              <List size={16} />
            </button>
          </div>
        ...
        {/* Filters */}
        <div className="flex flex-wrap gap-3.5 border-t border-slate-800/80 pt-4">
          {/* Status Filter */}
          ...
          {/* Health Filter */}
          <div className="flex items-center gap-2 text-xs">
            <span className="text-slate-500 font-bold uppercase tracking-wider">Health:</span>
            <select
              value={healthFilter}
      ...
      {filteredAssets.length > 0 ? (
            ...
            {filteredAssets.map((asset) => (
              ...
              >
                ...
                <div className="flex justify-between items-start gap-2">
                  <div>
                    <span className="text-[10px] font-mono font-bold bg-slate-900 text-slate-400 px-2 py-0.5 rounded border border-slate-850/80">
                      {asset.id}
                    </span>
                    <h4 className="text-base font-bold text-white mt-2 group-hover:text-violet-400 transition-colors">
                      {asset.name}
                    </h4>
                    <p className="text-xs text-slate-500 font-medium">{asset.category}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3.5 border-y border-slate-850/80 my-4 py-3.5 text-xs">
                  <div>
                    <span className="text-slate-500 font-bold block uppercase tracking-wider text-[9px]">Intel Health</span>
                    <div className="mt-1">{getHealthBadge(asset.healthScore)}</div>
                  </div>
                  <div>
                    <span className="text-slate-500 font-bold block uppercase tracking-wider text-[9px]">Utilization</span>
                    <div className="mt-1 font-semibold text-white font-mono">{asset.utilization}%</div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between text-xs mt-auto">
                  <div>
                    {asset.status === 'Allocated' ? (
                      ...
                      <div>
                        <span className="text-[9px] text-slate-500 block uppercase tracking-wider">Cost</span>
          ...
          <div className="glass-card rounded-2xl overflow-hidden border border-slate-800/80">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-900/60 border-b border-slate-800/80 text-[10px] text-slate-400 uppercase tracking-widest font-bold">
                  ...
                  {filteredAssets.map((asset) => (
                    <tr key={asset.id} className="hover:bg-slate-900/35 transition-colors">
                      ...
                      <td className="p-4">
                        ...
                        <span className="text-slate-500 text-xs font-medium">{asset.category}</span>
                      </td>
        ...
        <div className="flex flex-col items-center justify-center py-16 bg-slate-900/10 rounded-2xl border border-dashed border-slate-800">
          ...
          <h3 className="text-base font-bold text-slate-300">No assets match</h3>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full glass-input rounded-xl px-4 py-2.5 text-sm text-slate-100 placeholder-slate-500 focus:ring-1 focus:ring-violet-500 focus:border-violet-500 focus:outline-none"
                    value={newAsset.name}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      className="w-full glass-input rounded-xl px-3.5 py-2.5 text-sm text-slate-200 focus:ring-1 focus:ring-violet-500 focus:border-violet-500 focus:outline-none"
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      required
                      className="w-full glass-input rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:ring-1 focus:ring-violet-500 focus:border-violet-500 focus:outline-none"
